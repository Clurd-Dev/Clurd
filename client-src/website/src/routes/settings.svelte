	...
	onMount(async () => {
		{
			...
			os =
				information.system_name.replace('Some("', '').replace('")', '') +
				' ' +
				information.system_version.replace('Some("', '').replace('")', '') +
				' ' +
				information.kernel_version.replace('Some("', '').replace('")', '') +
				' ';
...
<div align="center">
	<fieldset class="uk-fieldset container">
		...
		<br />
		<p>
			This are the current settings for Clurd, if you want to change, modify the file config.toml in
			Clurd installation dir
		</p>
		<br />
		<div class="uk-margin">
			<p>Current path:</p>
