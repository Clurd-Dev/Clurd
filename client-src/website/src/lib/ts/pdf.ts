export class ComponentWindow {
	...
	appendCss() {
		...
		const parentStyleElements = Array.from(
			window.document.querySelectorAll('link[rel="stylesheet"][href^="client/"], style')
		);
	...
	attachComponent(componentClass, options) {
		...
		_component = new componentClass(
			Object.assign(Object.assign({}, options), { target: window.container })
		);
